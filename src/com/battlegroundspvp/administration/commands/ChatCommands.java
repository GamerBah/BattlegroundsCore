        ...
        if (cmd.getName().equalsIgnoreCase("clearchat")) {
            if (!gameProfile.hasRank(Rank.MODERATOR)) {
                ...
                for (Player p : Bukkit.getOnlinePlayers()) {
                    p.sendMessage(new ColorBuilder(ChatColor.RED).bold().create() + "The chat has been cleared by " + player.getName() + "!");
                    p.sendMessage(" ");
        ...
        if (cmd.getName().equalsIgnoreCase("lockchat")) {
            if (!gameProfile.hasRank(Rank.MODERATOR)) {
                ...
                if (!chatSilenced) {
                    ...
                    for (Player p : Bukkit.getOnlinePlayers()) {
                        p.sendMessage(" ");
                        p.sendMessage(new ColorBuilder(ChatColor.RED).bold().create() + "The chat has been locked by " + player.getName() + "!");
                        p.sendMessage(" ");
                        EventSound.playSound(p, EventSound.ACTION_SUCCESS);
                    }
                } else {
                    ...
                    for (Player p : Bukkit.getOnlinePlayers()) {
                        p.sendMessage(" ");
                        p.sendMessage(new ColorBuilder(ChatColor.RED).bold().create() + "All chat has been re-enabled!");
                        p.sendMessage(" ");
                        EventSound.playSound(p, EventSound.ACTION_SUCCESS);
                    }
                }
