    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args[0].equalsIgnoreCase("add")) {
            if (adding.contains(player)) {
                ...
                player.sendMessage(ChatColor.YELLOW + "Place an Enderchest onto an End Portal Frame to add a Crate location...");
            ...
            if (removing.contains(player)) {
                ...
                player.sendMessage(ChatColor.YELLOW + "Break an Enderchest that's on an End Portal Frame to remove a Crate location...");
                EventSound.playSound(player, EventSound.ACTION_FAIL);
                return true;
            }
            removing.add(player);
