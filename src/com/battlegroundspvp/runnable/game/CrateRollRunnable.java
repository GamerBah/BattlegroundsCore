...
public class CrateRollRunnable implements Runnable {
    ...
    public CrateRollRunnable(BattlegroundsCore plugin, Player player, Rarity rarity, Location location) {
        ...
        this.task = BattleCrateManager.open(battleCrate);
                    ...
                    if (timer <= 2) {
                        ...
                        if (reward.getServerType() == Cosmetic.ServerType.LOBBY) {
                            ...
                            if (gameProfile.getCosmetics().contains(reward.getId())) {
                                ...
                                if (player.isOnline())
                                    player.sendMessage(ChatColor.GRAY + "You received " + reward.getRarity().getColor() + (reward.getRarity() == Rarity.EPIC
                                            || reward.getRarity() == Rarity.LEGENDARY ? "" + ChatColor.BOLD : "") + reward.getName() + reward.getRarity().getColor()
                                            + " (" + reward.getRarity().getName() + ") " + ChatColor.GRAY + " for use in KitPvP!");
                            }
                        }
                        ...
                        for (Player players : Bukkit.getOnlinePlayers()) {
                            PacketPlayOutBlockAction packet = new PacketPlayOutBlockAction(new BlockPosition(location.getX(),
                                    location.getY(), location.getZ()), CraftMagicNumbers.getBlock(location.getBlock()), 0, 0);
