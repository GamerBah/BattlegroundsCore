...
public class PanelRunnables implements Runnable {
    ...
    public void run() {
        while (true) {
            ...
            BattlegroundsCore.getGameProfiles().stream().filter(gameProfile -> gameProfile.hasRank(Rank.HELPER)).forEach(gameProfile -> staff.add(gameProfile.getId()));
            int status = 0;
            ...
            try {
                ...
                try {
                    ...
                    serverData.setOnlineStaff(staff.toString());
