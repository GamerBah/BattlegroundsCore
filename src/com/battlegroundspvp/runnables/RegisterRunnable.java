    ...
    private boolean checkRegistration(Player player) {
        ...
        Future<Boolean> result = executorService.submit(() -> {
            ...
            Future<Boolean> future = Bukkit.getServer().getScheduler().callSyncMethod(plugin, () -> {
                if (registered) {
                    plugin.getGameProfile(player.getUniqueId()).setPassword(session.createQuery("from GameProfilesEntity where uuid = :uuid", GameProfilesEntity.class)
                            .setParameter("uuid", player.getUniqueId().toString()).getSingleResult().getPassword());
                    plugin.getGameProfile(player.getUniqueId()).setEmail(session.createQuery("from GameProfilesEntity where uuid = :uuid", GameProfilesEntity.class)
                    ...
                    player.sendMessage(" ");
                    player.sendMessage(new ColorBuilder(ChatColor.GREEN).bold().create() + "Registration complete! " + ChatColor.GRAY + "Welcome to Battlegrounds!");
                    ...
                    Bukkit.getServer().getPluginManager().callEvent(new PlayerJoinEvent(player, new ColorBuilder(ChatColor.GOLD).bold().create() + "New! "
                            + new ColorBuilder(ChatColor.DARK_GRAY).bold().create() + "[" + new ColorBuilder(ChatColor.GREEN).bold().create() + "+"
                            + new ColorBuilder(ChatColor.DARK_GRAY).bold().create() + "] " + ChatColor.WHITE + player.getName()));
                    return true;
                }
                return false;
            });
