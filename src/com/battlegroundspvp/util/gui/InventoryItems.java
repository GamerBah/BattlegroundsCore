...
public class InventoryItems {
    ...
    public static ItemBuilder punishItem(Punishment punishment) {
        GameProfile gameProfile = GameProfileManager.getGameProfile(punishment.getEnforcerId());
        ...
        if (gameProfile != null) {
            ...
            item = new ItemBuilder(Material.MAP).name(ChatColor.AQUA + punishment.getDate().minusHours(9).format(DateTimeFormatter.ofPattern("MMM d, yyyy 'at' h:mm a '(PST)'")))
    ...
    public static ItemBuilder crateItem(Player player, Rarity rarity, boolean buying, @Nullable Location location) {
        ...
        ItemBuilder crate = null;
        if (gameProfile != null) {
            crate = new ItemBuilder(Material.ENDER_CHEST).name(rarity.getCrateName()).lore(" ");
