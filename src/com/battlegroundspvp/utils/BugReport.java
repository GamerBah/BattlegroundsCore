...
public class BugReport {
    ...
    @Getter
    public enum Status {
        WAITING(0, "Waiting", new ColorBuilder(ChatColor.YELLOW).bold().create()),
        CONFIRMED(1, "Confirmed", new ColorBuilder(ChatColor.AQUA).bold().create()),
        RESOLVED(2, "Resolved", new ColorBuilder(ChatColor.GREEN).bold().create()),
        UNREPRODUCIBLE(3, "Unable to Reproduce", new ColorBuilder(ChatColor.RED).bold().create()),
        NEEDS_INFO(4, "Needs More Information", new ColorBuilder(ChatColor.GOLD).bold().create());
    ...
    public enum Priority {
        LOW(0, "Low", new ColorBuilder(ChatColor.GREEN).bold().create()),
        NORMAL(1, "Normal", new ColorBuilder(ChatColor.YELLOW).bold().create()),
        MEDIUM(2, "Medium", new ColorBuilder(ChatColor.GOLD).bold().create()),
        HIGH(3, "High", new ColorBuilder(ChatColor.RED).bold().create());
